...
export async function getStaticProps() {
  ...
  const vehiclesData = vehicleData.data.map(vehicle => {
    ...
    // Apply Kalman Filter to coordinates
    const kalmanFilter = new KalmanFilter({ Q: 0.01, R: 4 });
    
    // Raw positions for comparison (optional)
    const rawPositions = vehicleCoords.map(coord => [coord.latitude, coord.longitude]);
    
    // Apply Kalman filter to positions
    const filteredPositions = [];
    let lastTimestamp = null;
    
    vehicleCoords.forEach(coord => {
      const currentTimestamp = new Date(coord.timestamp);
      // Calculate time difference in seconds for more accurate Kalman filtering
      const deltaTime = lastTimestamp ? 
        (currentTimestamp - lastTimestamp) / 1000 : // Convert ms to seconds
    ...
    return {
      ...
      timestamps: vehicleCoords.map(coord => coord.timestamp)
    };
  });
