...
// Misalkan sudah dideklarasikan: ini seharusnya datang dari server untuk dipakai di klien untuk validasi (hanya contoh)
const existingGpsTrackerIDs = ['GPS123', 'GPS124', 'GPS125'];
...
const AddVehiclePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Memeriksa duplikasi GPS Tracker ID
    if (existingGpsTrackerIDs.includes(formData.gps_tracker_id)) {
      alert('GPS Tracker ID already exists. Please use a different ID.');
      return; // menghentikan proses jika ID sudah ada
    }
