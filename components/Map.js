...
export default function Map({ vehicles, useFilteredData = true }) {
  ...
  return (
    <div className="relative">
        ...
        {vehicles && vehicles.map(vehicle => {
          ...
          return (
            <div key={vehicle.id}>
                  ...
                  >
                    <Popup>
                      ...
                      {vehicle.timestamps && vehicle.timestamps.length > 0 
                        ...
                        : 'No timestamp'}
                      <br />
                      Lat: {filteredPositions[filteredPositions.length - 1][0].toFixed(6)}
                      <br />
                      Lng: {filteredPositions[filteredPositions.length - 1][1].toFixed(6)}
                    </Popup>
                  </Marker>
                </>
                  ...
                  >
                    <Popup>
                      ...
                      {vehicle.timestamps && vehicle.timestamps.length > 0 
                        ...
                        : 'No timestamp'}
                      <br />
                      Lat: {rawPositions[rawPositions.length - 1][0].toFixed(6)}
                      <br />
                      Lng: {rawPositions[rawPositions.length - 1][1].toFixed(6)}
                    </Popup>
                  </Marker>
                </>
              ) : null}
              ...
              {useFilteredData && rawPositions.length > 0 && filteredPositions.length > 0 && (
      ...
      <div className="absolute bottom-4 right-4 bg-gray-800 bg-opacity-80 p-2 rounded-md text-white text-xs z-[1000]">
        ...
        {vehicles && vehicles.length > 0 && vehicles[0].timestamps && vehicles[0].timestamps.length > 0 && (
          <div className="mt-2 pt-2 border-t border-gray-600">
            <div>Last update:</div>
            <div>{new Date(vehicles[0].timestamps[vehicles[0].timestamps.length - 1]).toLocaleString()}</div>
          </div>
        )}
      </div>
    </div>
