...
export default function TambahKendaraan() {
  ...
  return (
    <main className="p-8">
      <h1 className="text-2xl font-bold mb-6 text-white">Tambah Kendaraan Baru</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        {['merek', 'model', 'nomor_kendaraan', 'tahun_pembuatan', 'warna', 'jenis_kendaraan', 'pemilik'].map((field) => (
          <div key={field}>
            <label className="block text-white capitalize font-medium mb-1">{field.replace('_', ' ')}</label>
            <input 
              type="text" 
              name={field} 
              value={form[field]} 
